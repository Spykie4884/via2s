...
<div id="content">
	<div class="content_item">
		...
		if(isset($_POST['choix'])
			&& (($_POST['choix']) != ' '))
		{
			$resultats=$bdd->query('SELECT * FROM utilisateurs WHERE user_name="' . $_POST['choix'] . '"');
			$donnee = $resultats->fetch();
			$_SESSION['TMP'] = $donnee['user_email'];
		?>
			<center>
				<table class="table_separee">
					...
					<tr>
						<th>Statut</th>
						<td><?php echo $donnee['user_statut']; ?></td>
					</tr>
				</table>
		...
		<?php
					...
					&& isset($_SESSION['TMP']) && ($_SESSION['TMP']!=''))
				{
					...
					$q = $bdd->prepare("UPDATE utilisateurs SET valide=? WHERE user_email=?");
					$q->execute(array($valide,$_SESSION['TMP']));
				...
				else
				{
						...
						&& isset($_SESSION['TMP']) && ($_SESSION['TMP']!=''))
					{
						...
						$bdd = bdd_connexion();
						echo 'cou' . $_SESSION['TMP'];
						$q = $bdd->prepare("DELETE FROM utilisateurs WHERE user_email=?");
